function Me4mouthOPENandmothSEMI = importfile0302_openMOUTH(workbookFile, sheetName, dataLines)
%IMPORTFILE Import data from a spreadsheet
%  ME4MOUTHOPENANDMOTHSEMI = IMPORTFILE(FILE) reads data from the first
%  worksheet in the Microsoft Excel spreadsheet file named FILE.
%  Returns the numeric data.
%
%  ME4MOUTHOPENANDMOTHSEMI = IMPORTFILE(FILE, SHEET) reads from the
%  specified worksheet.
%
%  ME4MOUTHOPENANDMOTHSEMI = IMPORTFILE(FILE, SHEET, DATALINES) reads
%  from the specified worksheet for the specified row interval(s).
%  Specify DATALINES as a positive scalar integer or a N-by-2 array of
%  positive scalar integers for dis-contiguous row intervals.
%
%  Example:
%  Me4mouthOPENandmothSEMI = importfile0302_openMOUTH("C:\Users\Alessandro\Desktop\Polimi\Chicago Double Degree\5. Thesis\MyCode\MATLAB_20210215\File JSON\2021_03_02-Me4_mouthOPEN and mothSEMI.xlsx", "2021_03_02-Me4_mouthOPEN and mo", [1, 3776]);
%
%  See also READTABLE.
%
% Auto-generated by MATLAB on 02-Mar-2021 14:02:31

%% Input handling

% If no sheet is specified, read first sheet
if nargin == 1 || isempty(sheetName)
    sheetName = 1;
end

% If row start and end points are not specified, define defaults
if nargin <= 2
    dataLines = [1, 3776];
end

%% Setup the Import Options
opts = spreadsheetImportOptions("NumVariables", 2);

% Specify sheet and range
opts.Sheet = sheetName;
opts.DataRange = "A" + dataLines(1, 1) + ":B" + dataLines(1, 2);

% Specify column names and types
opts.VariableNames = ["VarName1", "VarName2"];
opts.SelectedVariableNames = ["VarName1", "VarName2"];
opts.VariableTypes = ["double", "double"];

% Import the data
Me4mouthOPENandmothSEMI = readtable(workbookFile, opts, "UseExcel", false);

for idx = 2:size(dataLines, 1)
    opts.DataRange = "A" + dataLines(idx, 1) + ":B" + dataLines(idx, 2);
    tb = readtable(workbookFile, opts, "UseExcel", false);
    Me4mouthOPENandmothSEMI = [Me4mouthOPENandmothSEMI; tb]; %#ok<AGROW>
end

%% Convert to output type
Me4mouthOPENandmothSEMI = table2array(Me4mouthOPENandmothSEMI);
end